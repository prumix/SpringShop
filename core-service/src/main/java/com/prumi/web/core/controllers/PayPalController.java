...
import com.prumi.web.core.services.PayPalService;
...
public class PayPalController {
    ...
    public ResponseEntity<?> captureOrder(@PathVariable String payPalId) throws IOException {
        ...
        if ("COMPLETED".equals(payPalOrder.status())) {
            ...
            Optional<com.prumi.web.core.entities.Order> orderOptional = orderService.findById(orderId);
            orderOptional.get().setStatusOrder(StatusOrder.PAID);
            System.out.println(orderOptional.get().getStatusOrder());;
